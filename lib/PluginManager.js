 ...
 */
function checkPackageForPlugins(manager, moduleName, pkg, isLocal) {
...
module.exports = class PluginManager extends WeightedEmitter {
     ...
     * @async
     */
    index(done, searchDevDependencies) {
        const me = this;
        const pkg = findPackage();
        checkPackageForPlugins(this, pkg.name, pkg, true);
        ...
        require('child_process').exec(npmCmd, function(err, stdout, stderr) {
            ...
            for (let moduleName in modInf.dependencies) {
                ...
                checkPackageForPlugins(me, moduleName, pkg);
            }
